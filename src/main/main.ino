...
#error edit SdFatConfig.h .  READ THE COMMENT //Modifier le fichier SdFatConfig.h -> SPI_DRIVER_SELECT = 2
#endif
...
#define PALETTEDEPTH   8     // 256-colour Palette
...
void openAndDrawImage(){
  
    char namebuf[32] = "/";
    File file;
    
    strcpy(namebuf, (char *)"aubi.bmp");
    file = SD.open(namebuf);
    uint8_t ret;
    uint32_t start;
    if (file != NULL) {
      file.close();
